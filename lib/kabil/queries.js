...
export const useApplications = (
  ...
  return useQuery({
    ...
    enabled: !!jobId,
    refetchInterval: (q) => {
      // A live stream pushes a nudge the moment a score lands and the
      // RealtimeProvider invalidates this list — so don't poll while it's up.
      // As a fallback (stream offline/disabled), poll while some active card is
      // still awaiting the score for its current stage. The list item carries
      // no `pipeline_status`, so we key off the denormalized score columns:
      // a card sits in `hard_filter`/`vector_screen` with a null score until
      // the async scoring lands.
      if (!realtimeOffline) return false;
 ...
 * backend **hard-deletes** the application (cascading its scores + WhatsApp
 * conversation) and (re-)adds the candidate to the pool — so we refetch both
 * the board's applications and the pool. The candidate can be sourced back
 * onto the job afterwards with no "already applied" conflict.
 */
